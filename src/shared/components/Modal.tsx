  ...
  return (
    <div className="fixed inset-0 z-[1000]">
      ...
      <div className="absolute inset-0 flex items-start justify-center overflow-auto p-4">
        <div className={`w-full ${maxW} rounded-lg bg-white text-slate-900 shadow-xl`}>
          <div className="flex items-center justify-between border-b px-4 py-3">
            <h3 className="text-lg font-semibold">{title}</h3>
            <button onClick={onClose} className="rounded px-2 py-1 text-slate-500 hover:bg-slate-100">✕</button>
          </div>
          <div className="p-4">
            {children}
          </div>
